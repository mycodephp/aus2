...
class grade_report_grader extends grade_report {
    ...
    public function get_left_rows($displayaverages) {
        ...
        foreach ($this->users as $userid => $user) {
           ...
           // $usercell->text .= '<button type="button">status</button>';
            ...
            foreach ($extrafields as $field) {
                ...
                $get_status=$DB->get_record('grade_status',['userid'=>$user->id,'cid'=>$_GET['id']]);
if(empty($get_status)){
    $userrow->cells[] =' <select id="dropdownInput" class="dropdown-input">
    ...
    if($get_status->status=='notPending'){
        $notPending ="selected";
    }
    if($get_status->status=='notRequirement'){
        $notRequirement ="selected";
    }
    $userrow->cells[]=' <select id="dropdownInput" class="dropdown-input">
    <option value=""  disabled>Select an option</option>
    <option  '.$pending.' value="pending"   >pending</option>
    <option '.$notPending.' value="notPending">Not Pending</option>
    <option '.$notRequirement.' value="notRequirement">Not Requirement</option>
  </select>';
